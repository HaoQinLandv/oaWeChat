...
*/
    ...
    //叶某某改的
		$("#starDate").text(this.mOpts.startDate);
        $("#endDate").text(this.mOpts.endDate);
    // 初始化日期选择器面板的HTML代码串
    var wrapper = {
		gri :[
    ...
    // 把时间选择框放到页面中
    $(0 < $('#appendParent').length ? '#appendParent' : '.checkDate-dateRange').append(wrapper[this.mOpts.theme].join(''));
...
pickerDateRange.prototype.selectDate = function(ymd) {
    ...
    if(this.startDateId == this.dateInput) {
        ...
        // 更改对应输入框的值
        $('#' + this.dateInput).val(ymdFormat);
        $("#starDate").text($('#' + this.startDateId).val());
        $("#endDate").text($('#' + this.endDateId).val());
        // 切换输入框焦点
        this.dateInput = this.startDateId;
		this.removeCSS(0);
        this.addCSS(0);
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    //var today = new Date();//源码
    //var today = new Date('2018-01-19')//测试
    var today =  new Date(new Date()-24*60*60*1000)//QQ3099866改的 改为昨天,那么就可以选择今天开始入住啦
